...
%======================================================================
function output = treadmillSpeedConstraintsSigmoidImproved(obj,option,X,sym)
...
if strcmp(option,'confun')
    ...
    for iNode=1:(nNodesDur-1)
        ...
        v_left = v_left_curr + Kfx *((lfx2 - lfx1)/c) + Kfy*((lfy2 - lfy1)/c) + Kp*(obj.model.speed_left - v_left_curr) + Kd * ((-v_left_curr+ v_left_prev)/c);
        v_right = v_right_curr + Kfx*((rfx2 - rfx1)/c) + Kfy*((rfy2 - rfy1)/c) + Kp*(obj.model.speed_right - v_right_curr) + Kd * ((-v_right_curr+ v_right_prev)/c);
...
elseif strcmp(option,'jacobian')
    ...
    for iNode = 1:(nNodesDur-1)
        ...
        output(ic(1), idxFxToeLinX(delayed_index2)) = sigmoid_left*m*Kfx/c;
        output(ic(1), idxFxHeelLinX(delayed_index2)) = sigmoid_left*m*Kfx / c;
        output(ic(1), idxFxToeLinX(delayed_index)) = -sigmoid_left*m*Kfx/c;
        output(ic(1), idxFxHeelLinX(delayed_index)) = -sigmoid_left*m*Kfx / c;


        %derivative of left belt wrt left GRFy (at states heel and toe)
        output(ic(1), idxFyToeLinX(delayed_index2)) = sigmoid_left*Kfy * m/c;
        output(ic(1), idxFyHeelLinX(delayed_index2)) = sigmoid_left*Kfy * m / c;
        output(ic(1), idxFyToeLinX(delayed_index)) = -m*sigmoid_left* Kfy/c;
        output(ic(1), idxFyHeelLinX(delayed_index)) = -m*sigmoid_left*Kfy /c;
        ...
        output(ic(2), idxFxToeRinX(delayed_index2)) = sigmoid_right*m*Kfx/c;
        output(ic(2), idxFxHeelRinX(delayed_index2)) = sigmoid_right*m*Kfx / c;
        output(ic(2), idxFxToeRinX(delayed_index)) = -sigmoid_right*m*Kfx/c;
        output(ic(2), idxFxHeelRinX(delayed_index)) = -sigmoid_right*m*Kfx / c;

        %derivative of right belt wrt right GRFy (at states heel and toe)
        output(ic(2), idxFyToeRinX(delayed_index2)) = Kfy * sigmoid_right*m/c;
        output(ic(2), idxFyHeelRinX(delayed_index2)) = Kfy *sigmoid_right* m / c;
        output(ic(2), idxFyToeRinX(delayed_index)) = -m*sigmoid_right* Kfy/c;
        output(ic(2), idxFyHeelRinX(delayed_index)) = -m*sigmoid_right*Kfy /c;
