...
if strcmp(option,'confun')
    ...
    for iNode=1:(nNodesDur-1)
        ...
        v_left = obj.model.speed_left + Kgrf *((lfx2 - lfx1)/c) + Kgrf*Kfy*((lfy2 - lfy1)/c) + Kpd*Kp*(obj.model.speed_left - v_left_curr) + Kpd*Kd * ((-v_left_curr+ v_left_prev)/c);
        v_right = obj.model.speed_right + Kgrf *((rfx2 - rfx1)/c) + Kgrf*Kfy*((rfy2 - rfy1)/c) + Kpd*Kp*(obj.model.speed_right - v_right_curr) + Kpd*Kd * ((-v_right_curr+ v_right_prev)/c);
...
elseif strcmp(option,'jacobian')
    ...
    for iNode = 1:(nNodesDur-1)
        ...
        output(ic(1), idxVLeft(iNode)) = sigmoid_left*(-Kpd*Kp - Kpd*Kd/c); % der from pd Part wrt v(n)
        output(ic(1), idxVLeft(mod(iNode - 2, nNodesDur-1)+1)) =  sigmoid_left*Kpd*Kd/c; %derivative wrt v(n-1)

        output(ic(2), idxVRight(iNode)) = sigmoid_right*( -Kpd*Kp - Kpd*Kd/c); % der from pd Part wrt v(n)
