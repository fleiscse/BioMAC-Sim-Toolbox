...
%======================================================================
function output = treadmillSpeedConstraintsSigmoid_sliding_window(obj,option,X,sym)
...
if strcmp(option,'confun')
    ...
    for iNode=1:(nNodesDur-1)
        ...
        delayed_index = mod(iNode - delay-2, nNodesDur-1) +1; %instead: use -22
        ...
        v_left = v_left_curr + Kgrf *((lfx2 - lfx1)/(3*c)) + Kgrf*Kfy*((lfy2 - lfy1)/(3*c)) + Kpd*Kp*(obj.model.speed_left - v_left_curr) + Kpd*Kd * ((-v_left_curr+ v_left_prev)/c);
        v_right = v_right_curr + Kgrf *((rfx2 - rfx1)/(3*c)) + Kgrf*Kfy*((rfy2 - rfy1)/(3*c)) + Kpd*Kp*(obj.model.speed_right - v_right_curr) + Kpd*Kd * ((-v_right_curr+ v_right_prev)/c);
...
elseif strcmp(option,'jacobian')
    ...
    for iNode = 1:(nNodesDur-1)
        ...
        output(ic(1), idxFxToeLinX(delayed_index2)) = sigmoid_left*m*Kgrf/(3*c);
        output(ic(1), idxFxHeelLinX(delayed_index2)) = sigmoid_left*m*Kgrf / (3*c);
        output(ic(1), idxFxToeLinX(delayed_index)) = -sigmoid_left*m*Kgrf/(3*c);
        output(ic(1), idxFxHeelLinX(delayed_index)) = -sigmoid_left*m*Kgrf / (3*c);


        %derivative of left belt wrt left GRFy (at states heel and toe)
        output(ic(1), idxFyToeLinX(delayed_index2)) = sigmoid_left*Kfy * m*Kgrf/(3*c);
        output(ic(1), idxFyHeelLinX(delayed_index2)) = sigmoid_left*Kfy * m*Kgrf / (3*c);
        output(ic(1), idxFyToeLinX(delayed_index)) = -m*sigmoid_left*Kgrf* Kfy/(3*c);
        output(ic(1), idxFyHeelLinX(delayed_index)) = -m*sigmoid_left*Kgrf*Kfy /(3*c);
        ...
        output(ic(2), idxFxToeRinX(delayed_index2)) = sigmoid_right*m*Kgrf/(3*c);
        output(ic(2), idxFxHeelRinX(delayed_index2)) = sigmoid_right*m*Kgrf / (3*c);
        output(ic(2), idxFxToeRinX(delayed_index)) = -sigmoid_right*m*Kgrf/(3*c);
        output(ic(2), idxFxHeelRinX(delayed_index)) = -sigmoid_right*m*Kgrf / (3*c);

        %derivative of right belt wrt right GRFy (at states heel and toe)
        output(ic(2), idxFyToeRinX(delayed_index2)) = Kfy * sigmoid_right*m*Kgrf/(3*c);
        output(ic(2), idxFyHeelRinX(delayed_index2)) = Kfy *sigmoid_right* m*Kgrf / (3*c);
        output(ic(2), idxFyToeRinX(delayed_index)) = -m*sigmoid_right*Kgrf* Kfy/(3*c);
        output(ic(2), idxFyHeelRinX(delayed_index)) = -m*sigmoid_right*Kgrf*Kfy /(3*c);
